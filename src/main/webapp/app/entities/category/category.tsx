    ...
    return (
      <div>
        <h2 id="category-heading">
          <Translate contentKey="tdPortalApp.category.home.title">Categories</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp;
            <Translate contentKey="tdPortalApp.category.home.createLabel">Create new Category</Translate>
          </Link>
        </h2>
              ...
              {categoryList.map((category, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {category.image ? (
                      <div>
                        <a onClick={openFile(category.imageContentType, category.image)}>
                          <img src={`data:${category.imageContentType};base64,${category.image}`} style={{ maxHeight: '30px' }} />
                          &nbsp;
                        </a>
                        <span>
                          {category.imageContentType}, {byteSize(category.image)}
                        </span>
                      </div>
                    ) : null}
                  </td>
